...
1765: (e, t, i) => {
    ...
    let u, _ = !1,
      p = null,
      m = {};
    const g = [/^widgetbar\.widget\.watchlist.+/, /.+quicks$/, /^widgetbar\.layout-settings$/,
      /^ChartSideToolbarWidget\.visible$/, /^onwidget\.watchlist$/, /^chart\.favoriteDrawings$/,
      /^chart\.favoriteDrawingsPosition$/, /^chart\.favoriteLibraryIndicators$/, /^loadChartDialog.favorites$/,
      /^ChartFavoriteDrawingToolbarWidget\.visible/, /^trading\.chart\.proterty$/,
      /^trading_floating_toolbar\.position$/, /^trading\.orderWidgetMode\./, /^symbolWatermark$/,
      /^pinereference\.size$/, /^pinereference\.position$/, /^hint\.+/, /^ChartDrawingToolbarWidget\.visible/
    ];
    g.push(/^savedwatch\..+/);
    const f = () => !p && l,
      y = e => d + e,
    ...
    function L(e) {
      const t = A(),
        i = [],
        s = new RegExp("^" + (o = e, o.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&")).replace(/\\\*\\\*/gi, ".+").replace(
          /\\\*/gi, "[^.]+") + "$", "gi");
      var o;
