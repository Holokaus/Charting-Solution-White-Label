...
21097: (e, t, i) => {
    ...
    var logger = i(9343),
      o = i(11417);
    const n = (0, logger.getLogger)("XWindowEvents");
    var r;
    ! function(e) {
      const t = "tvxwevents.",
        i = {};
      ...
      window.BroadcastChannel ? (logger = new BroadcastChannel("tvxwevents"), logger.addEventListener("message", (
        ...
        for (let i = 0; i < o.TVLocalStorage.length; i++) {
          ...
          logger.startsWith(t) && e.push(logger)
        }
        const logger = o.TVLocalStorage.length;
        for (const t of e) o.TVLocalStorage.removeItem(t);
        const r = performance.now() - i;
        n.logNormal(`Total keys amount in local storage on operation start: ${logger}`), n.logNormal(
          `Keys amount in local storage to be deleted: ${e.length}`), n.logNormal(
          `Keys to be deleted from local storage: ${JSON.stringify(e)}`), n.logNormal(
        ...
        if (null === logger || !n || !n.startsWith(t)) return;
