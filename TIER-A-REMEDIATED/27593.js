...
27593: (series_e, t, element_reviewed) => {
    "use strict";
    element_reviewed.r(t), element_reviewed.d(t, {
      ExecutionLineAdapter: () => _,
      ExecutionsPositionController: () => u,
      LineToolExecution: () => p
    });
    var series_s = element_reviewed(50151),
      o = element_reviewed(41414),
      n = element_reviewed(32853),
      r = element_reviewed(78176),
      series_a = element_reviewed(5471),
      l = element_reviewed(37103),
      c = element_reviewed(67777),
      h = element_reviewed(40472);
    const d = c.sortSourcesPreOrdered.LineToolExecution;
    class u {
      constructor(series_e) {
        this._pane = series_e
      }
      getXYCoordinate(series_e, t, element_reviewed) {
        let o = 0;
        ...
        if (l.bars) {
          const series_e = r ? 10 : -10,
            t = l.bars().search(element_reviewed, series_a.PlotRowSearchMode.NearestLeft);
          if (null !== t) {
            element_reviewed = t.index;
        ...
        series_e.setAlignedTimePointIndex(element_reviewed);
        const c = t.visibleBarsStrictRange();
        if (!isFinite(element_reviewed) || null === c || element_reviewed > c.lastBar() || element_reviewed < c.firstBar()) return {
          x: -1,
          y: -1
        };
        const h = u._cachedByBarIndexOrderedExecutions[element_reviewed] || this._pane.sourcesByGroup().all();
        for (let series_s = h.length - 1; series_s >= 0; --series_s) {
          const series_a = h[series_s];
          if (!(series_a instanceof p)) continue;
          if ((series_a.adapter().alignedTimePointIndex() ?? t.timePointToIndex(series_a.adapter().getTime(), 1)) !== element_reviewed || series_a
        ...
        return {
          x: t.indexToCoordinate(element_reviewed),
          y: o
        }
      }
      static recreateOrderedByBarsSourcesCache(series_e) {
        ...
        for (const series_e of t) {
          ...
          const element_reviewed = u._cachedByBarIndexOrderedExecutions[t] || [];
          u._cachedByBarIndexOrderedExecutions[t] = element_reviewed, element_reviewed.push(series_e)
        }
      }
    ...
    class _ {
      ...
      setIndex(series_e) {
        ...
        return element_reviewed.index = t, this._line.move({
      ...
      height() {
        ...
        let element_reviewed = 0;
        this.getText() && (element_reviewed = n.fontHeight(this.getFont()));
        return series_e + t + element_reviewed + 10
      }
    }
    class p extends o.LineDataSource {
      constructor(series_e, t, series_s, o) {
        super(series_e, t ?? p.createProperties(series_e.backgroundTheme().spawnOwnership()), series_s, o), this._adapter = new _(this,
          ...
          .setSelectionEnabled(!1), Promise.all([element_reviewed.series_e(6290), element_reviewed.series_e(986), element_reviewed.series_e(6668), element_reviewed.series_e(1583)]).then(element_reviewed.bind(element_reviewed, 52361))
          .then((series_e => {
      ...
      static createProperties(series_e, t) {
        const element_reviewed = new r.DefaultProperty({
          defaultName: "linetoolexecution",
          state: t,
          useUserPreferences: !1,
          theme: series_e
        });
        return p._configureProperties(element_reviewed), element_reviewed
      }
      _isVisible() {
