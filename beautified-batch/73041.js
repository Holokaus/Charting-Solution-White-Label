/**
 * Module 73041 - Auto-beautified from TradingView webpack bundle
 *
 * @module 73041
 * @date 2026-04-23
 * @size 2186 bytes
 ...
 *
 * Dependencies: 24640, 33350
 *
 * Exports:
 *   - drawSelection (internal: r)
 *   - getSymbolCoordinatesInfo (internal: n)
 *
 * Next Steps:
...
function n(e) {
  ...
  for (let e = 0; e < n.length; e++) {
    ...
    if (f || (g = y), v && s > y.length) {
      p += y.length + (r && !c ? 0 : 1), f || (d += _);
      continue
    }
    f && (d -= _);
    const S = r && !c && y.length === s && v && !v.hidden;
    if (f && (g += " ", s = g.length), "center" === l)
      if (S) h = i / 2;
      else {
        const e = (0, o.measureText)(g, a).width,
          t = (0, o.measureText)(g.slice(0, s), a).width,
          n = i / 2;
        h = u ? n + e / 2 - t : n - e / 2 + t
      }
...
function r(e, t, i) {
  ...
  if (r.lineNumber === l.lineNumber) {
    ...
    for (let s = r.lineNumber; s <= l.lineNumber; s += 1) {
      ...
      const p = a(_.text, u);
      let S, b, w = !1;
      "center" === e.textAlign ? (S = o ? r.x : f ? m + p / 2 : m - p / 2, b = d ? l.x : f ? m - p / 2 : m + p / 2) : "right" === e.textAlign || f && "start" === e.textAlign || !f && "end" === e.textAlign ? (S = o ? r.x : h - p, b = d ? l.x : h, w = !0) : (S = o ? r.x : c, b = d ? l.x : c + p);
      let C = Math.min(S, b),
        T = Math.max(S, b);
      d || _.wrappedLinePart && !_.wrappedLineEnd || (w ? C -= t : T += t);
      const P = Math.round(C * y),
        x = Math.round(T * y),
        M = Math.round((r.y + i * g) * v),
        I = Math.round((r.y + (i + 1) * g) * v);
