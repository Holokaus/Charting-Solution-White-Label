 ...
 *
 * Dependencies: 2088, 2696, 3618, 3885, 4226, 4249, 4539, 5471, 9343, 10555, 11542, 11768, 11946, 12178, 13651, 16755, 17776, 18113, 22033, 22613, 24062, 24640, 27808, 28334, 29806, 30342, 30693, 31789, 32399, 32925, 35727, 36281, 36313, 36597, 37103, 40080, 40137, 40472, 40738, 43253, 43337, 43501, 45801, 46082, 46340, 46728, 47312, 48096, 48227, 48943, 49483, 50151, 50279, 50335, 50888, 51101, 51304, 51768, 52479, 52746, 52859, 52945, 52959, 53107, 54370, 54405, 55096, 57025, 58978, 59149, 60911, 61372, 62773, 62802, 63876, 64651, 65318, 67135, 67225, 67455, 67563, 69555, 69558, 70859, 70963, 72187, 72445, 73025, 75546, 75641, 76422, 77380, 78254, 78861, 82095, 82631, 85630, 86252, 86572, 87296, 87465, 88145, 88723, 88732, 89837, 89923, 91106, 92211, 93201, 94174, 94602, 94920, 95059, 97725, 97902, 98155, 99955
 *
 * Exports:
 *   - Series (internal: wi)
 *
...
function M(e) {
  ...
  if (e.description && t)
    if (2 === e.description.split("/").length) o = e.description.split("/")[1];
      ...
      e.description.split("'").filter((e => e.length)).forEach((e => {
        let t = [];
        t = e && ("/" === e[0] || /\d+\/\(?/.test(e)) ? [e] : e.split("/").filter((e => e.length)), o += t[2 === t.length ? 1 : 0]
      }))
    }
  else o = e.description ? e.description : e.symbol;
  if (e.ticker ? (s.title = e.ticker, s.description = I(o)) : s.title = I(o), e.interval && (s.interval = (0, x.translatedIntervalString)(e.interval)), t && e.description) {
  ...
  }(e)), e.sessionDescription && (s.sessionDescription = I(e.sessionDescription)), void 0 !== e.priceSource && (s.priceSource = I(e.priceSource)), e.adjustment && "dividends" === e.adjustment && (s.adjustment = w.t(null, {
    context: "adjustments"
...
class B extends P.StatusProviderBase {
  ...
  text() {
    return function(e) {
      ...
      return (e.ticker ? t.description : t.title) + (t.interval ? ", " + t.interval : "") + function(e, t = ", ") {
        return (e.provider ? `${t}${e.provider}` : "") + (e.listedExchange ? `${t}${e.listedExchange}` : "") + (e.chartStyle ? `${t}${e.chartStyle}` : "") + (e.adjustment ? `${t}${e.adjustment}` : "") + (e.backadjustment ? `${t}${e.backadjustment}` : "") + (e.settlement ? `${t}${e.settlement}` : "") + (e.sessionDescription ? `${t}${e.sessionDescription}` : "") + (e.priceSource ? `${t}${e.priceSource}` : "")
      }(t)
  ...
  _getTitleGenerationOptions() {
    ...
    const o = (D && e?.price_source_id ? this._series.model().availablePriceSources(this._series.getSymbolString()).name(e.price_source_id) : null) ?? void 0,
      n = this._series.getInputsProperties().state();
    17 === this._series.style() && (n.type = this._series.properties().childs().volFootprintStyle.childs().type.value());
...
function R(e, t) {
  if (null !== t) return "ticker" === e ? t.name : "long-description" === e && void 0 !== t.long_description ? t.long_description : (0, T.getTranslatedSymbolDescription)({
...
class N extends b.StatusView {
  constructor(e, t, i) {
    super(new B(e, t, i)), this._invalidated = !0, this._series = e, this._series.onRestarted().subscribe(this, this.update), this._series.dataEvents().symbolResolved().subscribe(this, this.update), this._series.dataEvents().completed().subscribe(this, this.update), this._series.boxSizeValue().subscribe(this.update.bind(this)), t.childs().symbolTextSource.subscribe(this, this.update)
  }
  getSeriesPrecision() {
    let e = 4;
...
class q extends F.DataWindowView {
  ...
  canShowItems() {
    return !!this._model.mainPane()?.maximized().value() || void 0 === this._model.panes().find((e => e.maximized().value()))
  }
  _updateImpl() {
    const e = this._getValuesProvider();
    this._valuesProvider !== e && (this._valuesProvider = e, this._items = this._valuesProvider.getItems().map((e => new F.DataWindowItem(e.id, e.title, "", e.unimportant))));
    const t = this._series.aliasSymbolInfo();
    if (t) {
      const e = [t.name];
      G || e.push((0, U.forceRTLStr)((0, x.translatedIntervalString)(this._series.interval()))), e.push((0, g.getSymbolListedExchange)(t)), this._header = e.join(` ${g.symbolTitleSeparator} `), this._title = t.description
    } else this._header = this._series.symbolOrAlias();
    let i = this._model.crosshairSource().appliedIndex();
    C.enabled("use_last_visible_bar_value_in_legend") && !(0, O.isNumber)(i) && (i = this._model.timeScale().visibleBarsStrictRange()?.lastBar() ?? NaN);
  ...
  _showLastPriceAndChangeOnly() {
    return j && (null === this._model.crosshairSource().pane || (0, z.isLineToolName)(H.tool.value()) || null !== this._model.lineBeingEdited())
  }
}
var $ = i(67455);
...
class Y extends q {
  constructor(e, t) {
    ...
    this._visibilityProperty = (0, $.combineProperty)(((e, t, i, s, o) => `${e}:${t}:${i}:${s}:${o}`), i.showBarChange.weakReference(), i.showSeriesOHLC.weakReference(), i.showVolume.weakReference(), i.showLastDayChange.weakReference(), i.showSeriesLegendCloseOnMobile.weakReference()), this._visibilityProperty.subscribe(this, this.update)
  }
  areValuesVisible() {
...
class ne {
  constructor(e, t) {
    this._extrapolatedData = [], this._cacheForFuture = !1, this._modelId = oe++, this._builderCache = null, this._uniqueId = (0, se.randomHashN)(6), this._resolution = t, this._symbolInfo = e, this._valid = Boolean(e.timezone) && Boolean(e.session), this._session = new ee.SessionInfo(e.timezone, e.session, e.session_holidays, e.corrections)
  }
  syncSourceTarget() {
    return {
  ...
  distance(e, t) {
    ...
    const n = e < t;
    if (1e3 * e === s && this._cacheForFuture === n || (this._extrapolatedData = [1e3 * e], i = 1, s = null, o = null), null === s || null !== o && 1e3 * t > o) {
  ...
  projectTime(e, t) {
    ...
    let i = this._extrapolatedData.length,
      ...
      o = null !== s ? this._extrapolatedData[i - 1] : null;
    const n = t >= 0;
    1e3 * e === s && this._cacheForFuture === n || (this._extrapolatedData = [1e3 * e], i = 1, s = null, o = null);
    const r = Math.abs(t);
    if (null === s || r >= i) {
      const s = (0, ie.extrapolateBarsFrontByCount)(this.barBuilder(), o || 1e3 * e, Math.sign(t) * (r - i + 1), !0);
      this._extrapolatedData = this._extrapolatedData.concat(s.times), i = this._extrapolatedData.length, this._cacheForFuture = n
    }
  ...
  barBuilder() {
    return null === this._builderCache && (this._builderCache = (0, te.newBarBuilder)(this._resolution, this._session, this._session)), this._builderCache
  }
}
var re = i(62773),
  ae = i(50888);
...
class he {
  constructor(e, t, i) {
    this._dataUpdatedInfoStatus = new re.WatchedObject(null), this._symbolInfo = e, this._status = t, this._updatePeriod = i, this._symbolInfo.subscribe(this._update.bind(this)), this._status.subscribe(this._update.bind(this)), this._updatePeriod.subscribe(this._update.bind(this))
  }
  destroy() {
...
class ue {
  constructor(e, t) {
    this._mainDataProblem = new re.WatchedObject(null), this._supportPortalProblems = new re.WatchedObject([]), this._allDataProblems = new re.WatchedObject([]), this._pushStreamHandler = null, this._destroyed = !1, this._quotesProvider = e, this._quotesProvider.quotesUpdate().subscribe(this, this._update.bind(this)), this._symbolInfo = t, this._symbolInfo.subscribe((e => {
      e || this._mainDataProblem.setValue(null)
    })), this._mainDataProblem.subscribe((() => this._updateAllDataProblems())), this._supportPortalProblems.subscribe((() => this._updateAllDataProblems())), this._requestSupportPortalProblems()
  }
  destroy() {
  ...
  _update(e) {
    void 0 === e.values || void 0 === e.values.data_problem ? this._resetStatus() : this._mainDataProblem.setValue((0, de.clone)(e.values.data_problem))
  }
...
class Ce extends me.PriceAxisView {
  ...
  updateCountdown() {
    this._countdownText() !== this._previousCountdown && (this.update((0, ve.sourceChangeEvent)(this._source.id())),
  ...
  _updateRendererData(e, t, i) {
    ...
    let l = a.showSeriesLastValue.value(),
      c = this._isCountdownEnabled() && s.showCountdown.value() && (12 === (h = s.style.value()) ? be : (0, g.isTimeBasedStyle)(h)) && (this._options.alwaysShowGlobalLast || n.contains(r.index));
    var h;
    ...
    if (this._options.useSolidBodyColor ? (i.background = m, i.borderColor = void 0) : (i.background = this._model.backgroundColorAtYPercentFromTop((i.fixedCoordinate ?? i.coordinate) / (0, o.ensureNotNull)(this._model.paneForSource(this._source)).height()), i.borderColor = m), i.coordinate = _.coordinate, i.floatCoordinate = _.floatCoordinate, l || c) {
      ...
      this._previousCountdown = o, e.thirdLine = o, i.thirdLineTextColor = (0, pe.generateColor)(i.textColor, 25), 0 === t.length && 0 === s.length && 0 === o.length || (e.visible = !0)
    }
...
class Me extends Ce {
  ...
  _updateRendererData(e, t, i) {
    ...
    if (8 === T.style.value() && T.haStyle.childs().showRealLastPrice.value()) {
      ...
      if (e.noData || e.color === i.background || (y = (0, pe.resetTransparency)(e.color)), !e.noData && !C.noData) {
        const t = e.index === C.index;
        S = t && g.showSymbolLabels.value(), b = g.seriesLastValueMode.value() === ge.PriceAxisLastValueMode.LastPriceAndPercentageValue, v = v && t, w = t && this._isCountdownEnabled() && T.showCountdown.value()
      }
    } else {
      const e = n.barColorer().barStyle(p.index, !0);
      y = (0, pe.resetTransparency)(e.barColor)
    }
    if (i.background = y, i.textColor = this.generateTextColor(y), i.secondLineTextColor = i.textColor, i.thirdLineTextColor = (0, pe.generateColor)(i.textColor, 25), i.coordinate = r.priceToCoordinate(l, f), e.visible = v || w, !C.noData) {
      const i = n.priceScale().isPercentage();
      C.formattedPriceAbsolute = r.formatPriceAbsolute(l), C.formattedPricePercentage = r.formatPricePercentage(l, f, {
        signPositive: !0
      }), C.text = i ? C.formattedPricePercentage : C.formattedPriceAbsolute, e.text = this._axisFirstLineText(C, v), e.secondLine = v && b ? i ? C.formattedPriceAbsolute : C.formattedPricePercentage : "", e.thirdLine = w ? this._countdownText() : "", t.text = this._paneText(S)
    }
    t.visible = S
  }
}
...
class Ve extends me.PriceAxisView {
  constructor(e, t, i, s) {
    super(), this._model = e, this._label = t, this._isVisible = i.labelVisible, this._backgroundColor = i.lineColor, this._getValue = s
  }
  _updateRendererData(e, t, i) {
    ...
    e.visible = !0, t.visible = !0, e.text = o.formatPriceAbsolute(r), t.text = this._label, i.coordinate = o.priceToCoordinate(r, n), i.background = l, i.textColor = this.generateTextColor(l)
  }
}
...
class je extends me.PriceAxisView {
  ...
  _updateRendererData(e, t, i) {
    ...
    e.visible = !0, t.visible = !0, e.text = o.formatPriceAbsolute(r), t.text = this._label, i.coordinate = o.priceToCoordinate(r, n), i.background = a.backgroundColor, i.textColor = a.textColor
  }
}
...
class Je extends Ye.SeriesValuesProvider {
  constructor(e, t) {
    super(e, t);
    const i = t.properties().childs().paneProperties.childs().legendProperties.childs();
    this._showBarChangeProp = i.showBarChange, this._showLastDayChangeProp = i.showLastDayChange, this._showSeriesOHLCProp = i.showSeriesOHLC, this._showVolumeProp = i.showVolume, this._showSeriesLegendCloseOnMobile = i.showSeriesLegendCloseOnMobile, this._seriesStyleProp = e.properties().childs().style;
    const s = this._emptyValues[0],
      o = this._emptyValues[1],
      n = this._emptyValues[2];
    s.title = Ze.open, o.title = Ze.high, n.title = Ze.low, s.unimportant = !0, o.unimportant = !0, n.unimportant = !0, this._emptyValues[6].title = "", this._emptyValues[8].title = "", this._emptyValues[4].title = ""
  }
  getValues(e) {
    ...
    const _ = (0, g.isPriceSourceStyle)(this._seriesStyleProp.value()),
      p = o && !_,
      m = o && _;
    if (t[0].visible = p && c && !d, t[1].visible = p && !d, t[2].visible = p && !d, t[3].visible = p && h, t[3].title = d ? "" : Ze.close, t[4].visible = m, t[6].visible &&= n, t[8].visible &&= r, 16 === i) {
      const e = this._series.properties().childs().hlcAreaStyle.childs();
      t[1].color = e.highLineColor.value(), t[2].color = e.lowLineColor.value(), t[3].color = e.closeLineColor.value()
    }
    return t
  }
  _showOHLC() {
...
class nt {
  ...
  draw(e, t) {
    ...
    e.arc(a, l, c, 0, 2 * Math.PI, !1), e.fill(), e.fillStyle = i.fillColor, e.beginPath(), e.arc(a, l, i.radius * s, 0, 2 * Math.PI, !1), e.fill(), e.lineWidth = n, e.strokeStyle = i.strokeColor, e.beginPath(), e.arc(a, l, i.radius * s + n / 2, 0, 2 * Math.PI, !1), e.stroke(), e.restore()
  }
  hitTest(e) {
    return null
  }
...
class ut {
  constructor(e) {
    this._renderer = new nt, this._invalidated = !0, this._stageInvalidated = !0, this._startTime = performance.now(), this._endTime = this._startTime - 1, this._prevRenderedPrice = null, this._series = e
  }
  update(e) {
  ...
  renderer(e) {
    return this._invalidated ? (this._updateImpl(e), this._invalidated = !1, this._stageInvalidated = !1) : this._stageInvalidated && (this._updateRendererDataStage(), this._stageInvalidated = !1), this._renderer
  }
  _updateImpl(e) {
    ...
    this._lastBaseColor = null === this._prevRenderedPrice || this._prevRenderedPrice === o.price ? r : this._prevRenderedPrice < o.price ? ct : ht;
...
class ft {
  constructor() {
    this._created = new tt.Delegate,
      this._modified = new tt.Delegate, this._loading = new tt.Delegate, this._completed = new tt.Delegate, this._error = new tt.Delegate, this._symbolError = new tt.Delegate, this._symbolResolved = new tt.Delegate, this._seriesError = new tt.Delegate, this._symbolNotPermitted = new tt.Delegate, this._symbolInvalid = new tt.Delegate, this._symbolGroupNotPermitted = new tt.Delegate, this._chartTypeNotPermitted = new tt.Delegate, this._intradaySpreadNotPermitted = new tt.Delegate, this._intradayExchangeNotPermitted = new tt.Delegate, this._customIntervalNotPermitted = new tt.Delegate, this._secondsIntervalNotPermitted = new tt.Delegate, this._ticksIntervalNotPermitted = new tt.Delegate, this._barReceived = new tt.Delegate, this._seriesTimeFrame = new tt.Delegate, this._dataUpdated = new tt.Delegate, this._unsupportedResolutionRequested = new tt.Delegate
  }
  destroy() {
...
function Ct(e) {
  return e.startDate ? e.endDate || e.count ? e.endDate ? ["from_to", e.startDate, e.endDate] : ["bar_count", e.startDate, (0, o.ensure)(e.count)] : ["from_to", e.startDate] : e.count || 300
}
...
class Pt {
  constructor(e, t, i, s) {
    this._extSymbol = null, this._lastResolvedSymbol = "", this._createSeriesOverriddenParams = 0, this._instanceId = null, this._symbolInstanceId = null, this._resolution = null, this._timeFrame = null, this._data = new d.SeriesData, this._dataEvents = new ft, this._status = St.Idle, this._turnaroundCounter = 1, this._boundOnGatewayIsConnectedChanged = this._onGatewayIsConnectedChanged.bind(this), this._ongoingDataUpdate = Promise.resolve(), this._gateway = e, this._turnaroundPrefix = t, this._createSeriesParams = Ct(i ?? {
      count: 300
  ...
  modifySeries(e, t, i = null, s = !1, n = null) {
    s && (vt.logNormal("Due to force flag clearing symbol & resolution to force re-requesting data."), this._extSymbol = null, this._resolution = null);
    const r = Tt(e),
    ...
    (h || d || null !== i) && (this._timeFrame = null, (h || d) && this._turnaroundCounter++, h && this._resolveSymbol(), this._gateway.modifySeries(this._instanceId, this.turnaround(), (0,
      o.ensureNotNull)(this._symbolInstanceId), this._resolution, n, i, this._onMessage.bind(this)), this._dataEvents.fireModified())
  }
  requestMoreData(e) {
  ...
  start() {
    this.isStarted() ? vt.logNormal("start: data source is already started, nothing to do") : ((0, o.assert)(null !== this._extSymbol, "symbol must be set before start"), (0, o.assert)(null !== this._resolution, "resolution must be set before start"), this._gateway.isConnected().value() ? this._createSeries() : this._changeStatusTo(St.AwaitingConnection))
  }
  stop() {
    this.isStarted() ? (null !== this._instanceId && (this._gateway.removeSeries(this._instanceId), this._instanceId = null), this._symbolInstanceId = null, this._changeStatusTo(St.Idle)) : vt.logNormal("stop: data source is already stopped, nothing to do")
  }
  instanceId() {
  ...
  symbolSameAsResolved(e) {
    return null !== this._extSymbol && (0, yt.symbolSameAsResolved)(e, Tt(this._extSymbol), this._lastResolvedSymbol)
  }
  resolvedSymbol() {
    return null === this._extSymbol ? null : Tt(this._extSymbol)
  }
  _resolveSymbol() {
    null !== this._extSymbol && (this._symbolInstanceId = this._gateway.resolveSymbol("sds_sym_" + bt++, (0, st.encodeExtendedSymbolOrGetSimpleSymbolString)(this._extSymbol), this._onMessage.bind(this)))
  }
  _clearDataImpl() {
    this._data.clear()
  }
  _changeStatusTo(e) {
    (0, o.assert)(this._status !== e, "Source and destination status should be distinct"), vt.logNormal(`Status changed from ${St[this._status]} to ${St[e]}`), this._status = e
  }
  _createSeries() {
    (0, o.assert)(this._status !== St.Active, 'Status should not be "Active" when creating a study'), this._instanceId = "sds_" + wt++, this._resolveSymbol();
    const e = this._createSeriesOverriddenParams || this._createSeriesParams;
    this._createSeriesOverriddenParams && (this._createSeriesOverriddenParams = 0), this._gateway.createSeries(this._instanceId, this.turnaround(), (0, o.ensureNotNull)(this._symbolInstanceId), (0, o.ensureNotNull)(this._resolution), e, this._timeFrame, this._onMessage.bind(this)), this._timeFrame = null, this._changeStatusTo(St.AwaitingFirstDataUpdate), this._dataEvents.fireCreated(this._instanceId)
  }
  ...
  _onGatewayDisconnected() {
    this._status !== St.Idle && this._status !== St.AwaitingConnection && (this._instanceId = null, this._changeStatusTo(St.AwaitingConnection)), this._turnaroundCounter = 1
  }
  _onMessage(e) {
    ...
    switch (e.method) {
      case "symbol_resolved": {
        ...
        if (t !== this._symbolInstanceId) {
          null !== this._symbolInstanceId && vt.logNormal(`Resolve for old symbol, expected: ${this._symbolInstanceId}, actual ${e.params[0]}`);
          break
        }
      ...
      case "symbol_error":
        if (e.params[0] !== this._symbolInstanceId) {
          null !== this._symbolInstanceId && vt.logNormal(`Symbol error for old symbol, expected: ${this._symbolInstanceId}, actual ${e.params[0]}`);
          break
        }
      ...
      case "series_timeframe": {
        ...
        if (!this._checkTurnaround(t, i)) {
          vt.logNormal(`Time frame for old data, expected: ${this._symbolInstanceId} (${this.turnaround()}), actual ${t} (${i})`);
          break
        }
      ...
      case "series_error": {
        const [t, i] = e.params;
        if (!this._checkTurnaround(t, i)) {
          vt.logNormal(`Series error for old data, expected: ${this._symbolInstanceId} (${this.turnaround()}), actual ${t} (${i})`);
          break
        }
      ...
      case "series_completed": {
        ...
        if (!this._checkTurnaround(t, s)) {
          vt.logNormal(`Series completed for old data, expected: ${this._symbolInstanceId} (${this.turnaround()}), actual ${t} (${s})`);
          break
        }
      ...
      case "data_update":
        if (!this._checkTurnaround(e.params.customId, e.params.turnaround)) {
          vt.logNormal(`Data update for old data, expected: ${this._symbolInstanceId} (${this.turnaround()}), actual ${e.params.customId} (${e.params.turnaround})`);
          break
        }
  ...
  _onSeriesError(e) {
    ...
    if (t.startsWith("study_not_auth:")) {
      ...
      "resolution_not_entitled" === t ? this._dataEvents.fireIntradayExchangeNotPermitted() : "custom_resolution" === t ? this._dataEvents.fireCustomIntervalNotPermitted((0, o.ensureNotNull)(this._resolution)) : "seconds_not_entitled" === t ? this._dataEvents.fireSecondsIntervalNotPermitted() : "ticks_not_entitled" === t && this._dataEvents.fireTicksIntervalNotPermitted()
    }
  ...
  _onSymbolResolved(e) {
    this._lastResolvedSymbol = (0, o.ensureNotNull)((0, f.extractSymbolNameFromSymbolInfo)(e, "")), this._dataEvents.fireSymbolResolved(e)
  }
  ...
  _onDataUnpacked(e, t, i) {
    ...
    this._dataEvents.fireDataUpdated(t, l, r, a, o), s !== this._data.bars().size() && null !== r && this._dataEvents.fireBarReceived(r)
  }
  _onSeriesLoading(e) {
...
class Lt extends It {
  ...
  _updateImpl() {
    ...
    this._lineRendererData.visible = !0, this._lineRendererData.y = s, this._lineRendererData.color = this._series.priceLineColor(i.color), this._lineRendererData.linewidth = e.priceLineWidth.value()
  }
}
var kt = i(64651),
  Et = i(88732),
...
class Ut extends Vt.SeriesCandlesPaneView {
  renderer() {
    ...
    return o.append(new zt.PaneRendererCandles(s)), this._model.selection().isSelected(this._source) && this._isMarkersEnabled && this._selectionData && o.append(new Ht.SelectionRenderer(this._selectionData)), o
  }
}
var jt = i(54405),
  Gt = i(43501),
...
class ni {
  ...
  _updateImpl() {
    ...
    const t = this._model.crosshairSource().appliedIndex(),
      ...
      n = !!this._model.mainPane()?.priceDataSources().some((e => (0, Kt.isStudy)(e) && "Volume@tv-basicstudies" === e.metaInfo().id)),
      r = (e, t) => 7 === t ? e || n : e;
...
function bi(e) {
  ...
  if (i) {
    ...
    if (S.seriesReadyStatuses.has(t) && (i.type === ti.StudyStatusType.Loading || i.type === ti.StudyStatusType.Undefined)) return 2
  }
  return t
}
class wi extends c.PriceDataSource {
    constructor(e, t, i, s) {
      t.removeProperty("currencyId"), super(e, "_seriesId"), this.isSeries = !0, this.requestingStyleIsNotSupported = new tt.Delegate, this.requestingStyleSupportRecovered = new tt.Delegate, this._paneView = null, this._futureBarsPaneView = null, this._projectionBarsPaneView = null, this._waterlineView = null, this._priceLineView = null, this._gotoDateView = null, this._baseHorizontalLineView = null, this._priceStep = null, this._symbolInfo = new pt.WatchedValue(null), this._isPrePostMarketPricesAvailableProperty = new m.Property(!1), this._isBackAdjustmentForbiddenProperty = new m.Property(!0), this._isSettlementAsCloseForbiddenProperty = new m.Property(!0), this._highLowPriceCache = new Map, this._averagePriceCache = new Map, this._prevClosePriceAxisView = null, this._priceScaleAboutToBeChanged = new tt.Delegate, this._onRestarted = new tt.Delegate, this._onStatusChanged = new tt.Delegate, this._tagsChanged = new tt.Delegate, this._intervalChanged = new tt.Delegate, this._sessionIdChanged = new tt.Delegate, this._requestMoreDataAvailable = !0, this._lineStyleLastPriceCirclePaneView = new ut(this), this._prevClosePriceLineView = null, this._dataPoweredBy = null, this._symbolResolvingActive = new pt.WatchedValue(!1), this._predictBars = 0, this._syncModel = null, this._data = null, this._lastCompleteFlags = null, this._haStyle = {
        ...
        this._overriddenLastSplits = new pt.WatchedValue(null), this._lastSplits = (0, ei.combine)(((e, t) => t || e), this._quotesLastSplits.weakReference(), this._overriddenLastSplits.weakReference()), this._boxSizeValue = new pt.WatchedValue, this._base = 100, this._pointValue = 1, this._formattingDeps = null, this._defaultFormatter = new v.PriceFormatter({
          priceScale: this._base
        }), this._formatter = new v.PriceFormatter({
          priceScale: this._base
        }), this._ignoreMinMoveFormatter = new v.PriceFormatter({
          priceScale: this._base
        }), this._ignoreMinMovePriceStep = null, this._lastBarCloseTime = null, this._onSessionIdPropertyChangedBound = this._onSessionIdPropertyChanged.bind(this), this._ignoreSessionIdProxyPropertyChanges = !1, this._textSourceIsAlwaysTickerRestrictionEnabled = !1, this._lastPriceAnimationActive = !1, this._currentSession = "out_of_session", this._onStyleChanged = new tt.Delegate, this._obsoleteZOrder = 0, this._seriesErrorMessage = null, this._seriesAlwaysFalseHibernatedVW = new pt.WatchedValue(!1), this._styleToRecover = null, this._precomputedBarStyles = new WeakMap, this._doNotShowLastAvailableBar = !1, this._gotoDateResultCleared = !1, this._endOfDataPaneView = null, this._pendingTimeRange = null, this._replaySubscriber = new pt.WatchedValue(null), this._symbolIntervalChanged = new tt.Delegate, this._isReplayResolutionAvailableForUser = null, this._onInReplayStateChanged = new tt.Delegate, this._onReplaySubscriberSeriesModified = new tt.Delegate, this._paramsNotApplicableForReplay = new tt.Delegate, this._onTimeFrameApplied = new tt.Delegate, this._seriesLoaded = !1, this._seriesCompleted = !1, this._status = new pt.WatchedValue({
          seriesStatus: 0
        }), this._prevRequestedInterval = "", this._isActingAsSymbolSource = new pt.WatchedValue(!0), this._unsupportedResolutionState = new pt.WatchedValue(null), this._countdownUpdateTimer = null, this._deferredDestroyed = (0, l.createDeferredPromise)(), this._volumePointWeights = null, this._studyBindings = null, this._lineColorAtYPercentFromTop = new Pe.GradientColorCache, this._savedLeftEdge = null, this._pendingScrollToLeftEdge = null, this._symbolSourceWV = new pt.WatchedValue(this), this._updateTimeScaleTimePointWeights = () => {
          0
        };
      const n = t.childs().interval;
      this._intervalObj = new pt.WatchedValue(Q.Interval.parse(n.value())), t.childs().interval.subscribe(this, (() => this._intervalObj.setValue(Q.Interval.parse(n.value())))), this._seriesStatus = (0, ei.combine)(bi, this._status.weakReference()), this._seriesSource = new Pt(e.chartApi(), "s", s);
      const r = this._seriesSource.dataEvents();
      r.symbolResolved().subscribe(this, this._onSymbolResolved), r.symbolError().subscribe(this, this._onSymbolError), r.seriesTimeFrame().subscribe(this, this._onSeriesTimeFrame), r.seriesError().subscribe(this, this._onSeriesError), r.loading().subscribe(this, this._onSeriesLoading), r.completed().subscribe(this, this._onSeriesCompleted), r.dataUpdated().subscribe(this, this._onDataUpdated), r.barReceived().subscribe(this, this._onBarReceived), r.unsupportedResolutionRequested().subscribe(this, this._onUnsupportedResolutionRequested), this._quotesProvider = new _e.QuotesProvider(void 0, e.collapsed().spawnOwnership()),
        this._quotesProvider.quotesUpdate().subscribe(this, this._onQuotesUpdate);
      const a = t.childs();
      if (t.hasChild("extendedHours")) {
        (0, o.ensureDefined)(a.extendedHours).value() && !t.hasChild("sessionId") && t.addChild("sessionId", new m.Property("extended")), t.removeProperty("extendedHours")
      }
      t.hasChild("sessionId") || t.addChild("sessionId", new m.Property(y.SubsessionId.Regular)), (0, Jt.allChartStyles)().includes(a.style.value()) || a.style.setValueSilently(2);
      const c = a.lineStyle.childs();
      if (a.lineStyle.hasChild("styleType")) {
        let e, t;
        const i = c.styleType.value();
        0 === i && (t = 14, e = a.lineWithMarkersStyle.childs()), 1 === i && (t = 15, e = a.steplineStyle.childs()), e && (e.color.setValueSilently(c.color.value()), e.linestyle.setValueSilently(c.linestyle.value()), e.linewidth.setValueSilently(c.linewidth.value()), e.priceSource.setValueSilently(c.priceSource.value())), void 0 !== t && 2 === a.style.value() && a.style.setValue(t), a.lineStyle.removeProperty("styleType")
      }
      this._studyBindings?.bindToSeriesProperties(t), this._setProperties(t), this._sessionIdProxyProperty = new m.Property(a.sessionId.value()), a.sessionId.subscribe(this, (() => this._updateSessionIdProxyProperty())), this._sessionIdProxyProperty.subscribe(this, this._onSessionIdProxyPropertyChanged), this._symbolTextSourceProxyProperty = new m.Property, this._recalcSymbolTextSourceProxyProperty(), a.statusViewStyle.childs().symbolTextSource.subscribe(this, this._recalcSymbolTextSourceProxyProperty), this._symbolTextSourceProxyProperty.subscribe(this, (() => e.lightUpdate())), this._options = (0, de.merge)((0, de.clone)(yi), i), this._prevChartStyle = a.style.value(), this._priceAxisView = new Ce(this, e, {
      ...
      li || di || (h = new Ce(this, e, {
          ...
          alwaysShowGlobalLast: !1,
          useSolidBodyColor: !1
        })), this._priceLinePriceAxisView = new xe.SeriesPriceLineAxisView(this), this._priceLineAxisViews = [this._priceLinePriceAxisView], this._projectionPriceAxisView = new Me(this, e, {
          showCountdown: i.countdownEnabled
        }), this._priceAxisViews = [this._priceAxisView, this._projectionPriceAxisView], null !== h && this._priceAxisViews.push(h), this._panePriceAxisView = new Ie.PanePriceAxisView(this._priceAxisView, this, e), this._historyPricePanePriceAxisView = null !== h ? new Ie.PanePriceAxisView(h, this, e) : null, this._projectionPanePriceAxisView = new Ie.PanePriceAxisView(this._projectionPriceAxisView, this, e), this._labelPaneViews = [this._panePriceAxisView, this._projectionPanePriceAxisView], null !== this._historyPricePanePriceAxisView && this._labelPaneViews.push(this._historyPricePanePriceAxisView), this._highLowAvgPaneViews = [], this._averagePaneViews = [], ui && this._createAverageViews(), this._createHighLowAvgViews(), this._subscribeRestartToSessionIdChange(), a.visible.subscribe(this, this._updateLastPriceAnimationActive), this._updateLastPriceAnimationActive(), a.minTick.subscribe(this, this._onFormatterPropsChanged), this._dataWindowView = new q(this, e), this._legendView = new Y(this, e), this._statusView = new N(this, a.statusViewStyle), this._floatingTooltipView = new ni(e),
        this._marketStatusModel = new X.MarketStatusModel(this._quotesProvider), this._dataUpdatedModeModel = ci ? new he(this._symbolInfo.spawn().ownership(), this._seriesStatus.spawn().ownership(), (0, $.createWVFromGetterAndSubscription)((() => this._lastCompleteFlags?.rt_update_period), r.completed()).ownership()) : null, this._dataProblemModel = new ue(this._quotesProvider, this._symbolInfo.spawn().ownership()), this._symbolResolvingActive.subscribe((() => e.realignLineTools())), this._intervalChanged.subscribe(this, (() => e.realignLineTools())), this._intervalChanged.subscribe(this, (() => this._checkChartStyle())), this._studyBindings?.syncStudy(this.style()), this.createPaneView(), this._properties.addExcludedKey("visible", 1), this._properties.addExcludedKey("currencyId", 1), this._futureBarsPaneView = null, this._priceLineView = null, this._baseHorizontalLineView = new Qt(this);
      for (const [, e] of Object.entries(S.STYLE_SHORT_NAMES)) this._properties.child(`${e}Style`)?.subscribe(this, (() => this.invalidateBarStylesCache()));
      const d = a.renkoStyle.childs().inputs.childs();
      d.boxSize.subscribe(this, (() => this._restartIfStyleMatches(4))), d.style.subscribe(this, (() => this._restartIfStyleMatches(4))), d.atrLength.subscribe(this, (() => this._restartIfStyleMatches(4))), d.percentageLTP.subscribe(this, (() => this._restartIfStyleMatches(4))), d.wicks.subscribe(this, (() => this._restartIfStyleMatches(4))), d.sources.subscribe(this, (() => this._restartIfStyleMatches(4))), a.pbStyle.childs().inputs.childs().lb.subscribe(this, (() => this._restartIfStyleMatches(7)));
      const u = a.kagiStyle.childs().inputs.childs();
      u.reversalAmount.subscribe(this, (() => this._restartIfStyleMatches(5))), u.style.subscribe(this, (() => this._restartIfStyleMatches(5))), u.atrLength.subscribe(this, (() => this._restartIfStyleMatches(5))), u.percentageLTP.subscribe(this, (() => this._restartIfStyleMatches(5)));
      const _ = a.pnfStyle.childs().inputs.childs();
      _.boxSize.subscribe(this, (() => this._restartIfStyleMatches(6))), _.reversalAmount.subscribe(this, (() => this._restartIfStyleMatches(6))), _.sources.subscribe(this, (() => this._restartIfStyleMatches(6))), _.style.subscribe(this, (() => this._restartIfStyleMatches(6))), _.atrLength.subscribe(this, (() => this._restartIfStyleMatches(6))), _.percentageLTP.subscribe(this, (() => this._restartIfStyleMatches(6))), _.oneStepBackBuilding.subscribe(this, (() => this._restartIfStyleMatches(6))), a.rangeStyle.childs().inputs.childs().phantomBars.subscribe(this, (() => this._restartIfStyleMatches(11))), this._isDWMProperty = new m.Property(this.isDWM()), this._onRestarted.subscribe(this, (() => this._isDWMProperty.setValue(this.isDWM()))), a.showCountdown.subscribe(this, this._onShowCountdownChanged), this._onShowCountdownChanged(a.showCountdown), this._recreatePriceFormattingDependencies(), a.lineStyle.childs().priceSource.subscribe(this, this._updateBarFunction), a.lineWithMarkersStyle.childs().priceSource.subscribe(this, this._updateBarFunction), a.steplineStyle.childs().priceSource.subscribe(this, this._updateBarFunction), a.areaStyle.childs().priceSource.subscribe(this, this._updateBarFunction),
        a.baselineStyle.childs().priceSource.subscribe(this, this._updateBarFunction), a.columnStyle.childs().priceSource.subscribe(this, this._updateBarFunction), this._updateBarFunction(), this._isInReplay = (0, ei.combine)((e => null !== e), this._replaySubscriber.weakReference()), this._valuesProvider = new Ye.SeriesValuesProvider(this, this.model()), this._legendValuesProvider = new Je(this, this.model()), this._tableViewValuesProvider = new ai.SeriesTableViewValuesProvider(this, this.model()), this._dataWindowValuesProvider = new Ye.SeriesValuesProvider(this, this.model()), this._valuesProvider = new Ye.SeriesValuesProvider(this, this.model()), this._chartFloatingTooltipValuesProvider = new Qe(this, this.model())
    }
    setId(e) {
      super.setId(e), (0, Te.getPersistentLogger)()?.addPersistentLogEntry(`Set series Id to ${e}`, n.LOGLEVEL.INFO, "chart.series")
    }
    ...
    barColorer() {
      ...
      for (let i = t.length - 1; i >= 0; i--) {
        const s = t[i].dataSources().filter(Kt.isStudy).filter((e => e.hasBarColorer() && !e.isSourceHidden())).sort(((e, t) => e.zorder() - t.zorder()));
        for (let t = 0; t < s.length; t++) {
          const i = (0, o.ensureNotNull)(s[t].barColorer());
          null === e ? e = i : e.pushBackBarColorer(i)
        }
      }
      return null === e ? e = new $t.SeriesBarColorer(this) : e.pushBackBarColorer(new $t.SeriesBarColorer(this)), this._barColorerCache = e, e
    }
    createPaneView() {
      this._paneView = null, this._projectionBarsPaneView = null, this._waterlineView = null, this._priceLineView = this.hasClosePrice() ? new Lt(this) : null;
    ...
    topPaneViews() {
      ...
      if (null !== this._gotoDateView && e.push(this._gotoDateView), this._lastPriceAnimationActive) {
        ...
        t.animationActive() && setTimeout((() => this._model.invalidate(_t.InvalidationMask.cursor())), 0), t.invalidateStage(), e.push(t)
      }
      return 0 !== e.length ? e : null
    }
    paneViews(e) {
      if (!this.properties().childs().visible.value() || !this._model.isSnapshot() && !this._wasCompletedBefore()) return null;
      ...
      return i.push(...this._studyBindings?.paneViews(e) ?? []), t && (this._endOfDataPaneView && i.push(this._endOfDataPaneView), this._futureBarsPaneView && i.push(this._futureBarsPaneView), this._projectionBarsPaneView && i.push(this._projectionBarsPaneView), null !== this._waterlineView && i.push(this._waterlineView), null !== this._priceLineView && i.push(this._priceLineView), window.TradingView.printing && this._lastPriceAnimationActive && (this._lineStyleLastPriceCirclePaneView.stopAnimation(), i.push(this._lineStyleLastPriceCirclePaneView)), i.push(...this._highLowAvgPaneViews), i.push(...this._averagePaneViews)), i
    }
    ...
    setPriceScale(e) {
      this._priceScale !== e && (this._priceScaleAboutToBeChanged.fire(), this._priceScale = e,
        this._properties.removeProperty("priceAxisProperties"), this._properties.addChild("priceAxisProperties", e.properties()), this._properties.childs().priceAxisProperties.fireChanged(), (0, a.emit)("series_event", "price_scale_changed"), this._priceScaleChanged.fire(e))
    }
    priceScaleChanged() {
    ...
    applyPreferences(e) {
      ...
      this._properties.mergePreferences(o), this._properties.saveDefaults(), this.createPaneView(), this.invalidateBarStylesCache()
    }
    disconnect() {
    ...
    restart(e) {
      ...
      let t = null;
      this._pendingTimeRange && (t = this._pendingTimeRange, this._seriesSource.isStarted() || (this._seriesSource.setTimeframe(t), this._pendingTimeRange = null));
      const i = this._properties.childs().interval.value(),
        s = (0, Mt.getServerInterval)(i),
        o = this._getResolvingSymbolObject();
      this._seriesSource.canModifySeries(o, s) ? (this._seriesLoaded = !1, this._seriesCompleted = !1, this._lastCompleteFlags = null, this._onRestarted.fire(), this._setStatus(1), this._updateSymbolInfo(null), Q.Interval.isEqual(i, this._prevRequestedInterval) || this._notifyIntervalChanged(i), this._onBeforeModifySeries(this.getSymbolString(), i), this._onTimeFrameApplied.fire(t), this._data = null, this.model().recalcVisibleRangeStudies(h.RecalcVisibleRangeStudiesReason.SeriesRestart), this._seriesSource.modifySeries(o, s, t), this._seriesSource.isStarted() || this._seriesSource.start(), this._prevRequestedInterval = this.interval(), this._pendingTimeRange = null, this.updateAllViews((0, ve.sourceChangeEvent)(this.id())), this._model.lightUpdate()) : this._seriesSource.isStarted() || (this._updateSymbolInfo(null), this._seriesSource.start())
    }
    isSymbolInvalid() {
    ...
    invalidateBarStylesCache(e) {
      gi.logDebug("Invalidate style cache starting from " + e), this._clearStylePlot(this.bars(), e), this._clearStylePlot(this.nsBars())
    }
    isLoading() {
    ...
    clearData() {
      (0, o.assert)(null === this._data, "Cannot clear loaded data"), this._studyBindings?.clearData(), this._seriesSource.clearData(), this._dataRangeUpdated.fire({
        type: "full"
      })
    }
    nearestData(e, t) {
    ...
    requestMoreData(e) {
      ...
      if (this._predictBars < i && (this._predictBars = i, this._seriesSource.requestMoreTickmarks(i)), !this._requestMoreDataAvailable) return;
      const s = (0, o.ensureNotNull)(this.bars().firstIndex()),
        n = e || Math.max(s - t.firstBar(), s - (this._studyBindings?.requestMoreDataIndex() ?? s));
      if (n <= 0) return;
      if (!Number.isFinite(n)) return void gi.logWarn("requestMoreData: invalid bar count: " + n + ", visible bars: [" + t.firstBar() + ", " + t.lastBar() + "], last index: " + (0, o.ensureNotNull)(this.data().last()).index + ", predicted bars: " + this._predictBars + ", required bars:" + e);
      this._requestMoreDataAvailable = !1;
      const r = this._colorOnPrevBar() ? 2 : 0;
      this._seriesSource.requestMoreData(n + r), this._setStatus(2)
    }
    isNeedRestart(e) {
      return 5 !== this._status.value().seriesStatus && (void 0 === e && (e = this.properties().childs().style.value()), (0, g.styleChangeRequiresRestart)(this._prevChartStyle, e))
    }
    isStyleSupported(e) {
      return !0
    }
    ...
    onChartStyleChanged() {
      ...
      this._prevChartStyle !== e.childs().style.value() && this._studyBindings?.syncStudy(this.style()), this._prevChartStyle = e.childs().style.value(), this._onStyleChanged.fire(e.childs().style.value()), this.invalidateBarStylesCache(), this._updateLastPriceAnimationActive(), this._styleToRecover?.originalStyle !== this.style() && (this._styleToRecover = null), this._updateTimeScaleTimePointWeights()
    }
    ...
    loadDataTo(e) {
      ...
      this._onTimeFrameApplied.fire(e), this._seriesSource.modifySeries(this._getResolvingSymbolObject(), (0, Mt.getServerInterval)(t), e)
    }
    isInReplay() {
    ...
    lineColorAtYPercentFromTop(e) {
      ...
      return t ? t.colorType?.value() === ii.ColorType.Solid ? t.color.value() : this._lineColorAtYPercentFromTop.gradientColor((0, o.ensureDefined)(t.gradientStartColor).value(), (0, o.ensureDefined)(t.gradientEndColor).value(), e) : null
    }
    updateAllViews(e) {
      this._studyBindings?.updateAllViews(e), "hover-change" !== e.type && (this._paneView?.update(e), this._dataWindowView.update(), this._legendView.update(), this._statusView.update(), this._floatingTooltipView.update(e), this._averagePaneViews.forEach((t => t.update(e))), this._highLowAvgPaneViews.forEach((t => t.update(e))), this._labelPaneViews.forEach((t => t.update(e))), this._priceAxisViews.forEach((t => t.update(e))), this._priceLineAxisViews.forEach((t => t.update(e))), this._futureBarsPaneView?.update(e), this._projectionBarsPaneView?.update(e), this._waterlineView?.update(e), this._priceLineView?.update(e), this._gotoDateView?.update(e), this._endOfDataPaneView?.update(e), this._baseHorizontalLineView?.update(e), this._model.activeStrategySource().value()?.updateAllViews(e), this._model.replayStudyStrategy().value()?.updateAllViews(e), this._lineStyleLastPriceCirclePaneView.update(e))
    }
    styleStudyInfo(e) {
    ...
    setSymbolParams(e) {
      const {
        ...
        unitChanged: h,
        styleChanged: d
      } = this.compareSymbolParams(e), u = [vi("symbol", t, a), vi("interval", r, l), vi("currency", i, c), vi("unit", s, h), vi("style", n, d)].filter((e => null !== e)).join("; ");
      gi.logInfo(`Applying series symbol params: ${u}`);
      const _ = this.properties().childs();
      void 0 !== t && _.symbol.setValue(t), void 0 !== i && _.currencyId.setValue(i), void 0 !== s && _.unitId.setValue(s), l && _.interval.setValue((0, o.ensureDefined)(r)), void 0 !== n && d && _.style.setValue(n);
      let p = !1;
    ...
    state(e) {
      ...
      if (s.symbol = (0, g.symbolToSaveInState)(this.symbolInfo() ?? this._model.chartApi().lastSymbolResolveInfo(s.symbol), s.symbol), this._model.unitConversionEnabled() || (s.unitId = null), i.state = s, e) {
        let e = this.bars();
    ...
    restoreState(e, t) {
      t && this._setStatus(5), !this._model.unitConversionEnabled() && e.state && delete e.state.unitId, e.state && Q.Interval.isRange(e.state.interval) && (e.state.style = 11);
      const i = e.state;
      if (void 0 !== i) {
        const e = i.style,
          t = void 0;
      ...
      if (this._properties.mergeAndFire(e.state), this._prevChartStyle = this.properties().childs().style.value(), this.createPaneView(), t) {
        const t = e;
        this.restoreData(t.bars, t.nsBars, t.symbolInfo, t.rtPrice, t.boxSize, t.reversalAmount)
      }
      e.formattingDeps && (this._formattingDeps = e.formattingDeps, this._recreatePriceFormattingDependencies()), this._studyBindings?.restoreState(e.studyBindings ?? {}, t), this._studyBindings?.syncStudy(this.style())
    }
    restoreData(e, t, i, s, o, n) {
      this._updateStatus({
        seriesStatus: 5
      }), this._data = new d.SeriesData, this._data.bars().restoreState(e), this._data.nsBars().restoreState(t), this._updateSymbolInfo(i), this._data.lastProjectionPrice = s, this._data.boxSize = o;
      const r = this.properties().childs();
      o || (6 === r.style.value() ? this._data.boxSize = r.pnfStyle.childs().inputs.childs().boxSize.value() : 4 === r.style.value() && (this._data.boxSize = r.renkoStyle.childs().inputs.childs().boxSize.value())), this._data.reversalAmount = n, n || 5 === r.style.value() && (this._data.reversalAmount = r.kagiStyle.childs().inputs.childs().reversalAmount.value())
    }
    ...
    _setProperties(e) {
      e.hasChild("timeframe") || e.merge({
        ...
        shortName: ""
      }), e.hasChild("currencyId") || e.addChild("currencyId", new m.Property(null)), e.hasChild("unitId") || e.addChild("unitId", new m.Property(null)), this._properties = e;
      const t = e.childs();
    ...
    _onSymbolResolved(e) {
      this._seriesErrorMessage = null, this._updateSymbolInfo(e), this._model.updateSource(this), this._model.onWidget() || ((0, r.trackEvent)("Symbol", e.listed_exchange, e.name), (0, r.trackEvent)("Symbol Type", e.type, e.listed_exchange));
    ...
    _onSymbolError(e) {
      this._setStatus(4), this._properties.childs().shortName.setValue(this._properties.childs().symbol.value()), this._model.clearAllStudies(), this.updateAllViews((0, ve.sourceChangeEvent)(this.id())), this._model.updateSource(this), this._symbolResolvingActive.setValue(!1)
    }
    ...
    _onSeriesTimeFrame(e, t, i, s, o) {
      ...
      let r = t;
      if (void 0 === o) null !== i && "period-back" !== i.type || (r += this._model.studyAwareDefaultRightOffset());
    ...
    _onDataUpdated(e, t, i, s, n) {
      ...
      if (null !== this._volumePointWeights) {
        const e = t ? this.bars().fullRangeIterator() : i ? this.bars().rangeIterator(i.index, (0, o.ensureNotNull)(this.bars().lastIndex())) : null;
        if (null !== e)
          for (const t of e) this._volumePointWeights.setVolume(t.index, t.value[5])
      }
      t ? this._requestMoreDataAvailable = !0 : this._lastPriceAnimationActive && this._seriesLoaded && this._lineStyleLastPriceCirclePaneView.update((0, ve.sourceChangeEvent)(this.id())), this._lastBarCloseTime = e && e.closeTime || null, this._boxSizeValue.setValue(this.data().boxSize), this._statusView.update();
    ...
    _setStatus(e) {
      ...
      this._updateStatus(t), this._statusView.update(), this.model().updateSource(this), this._onStatusChanged.fire()
    }
    _onBarReceived(e) {
    ...
    _getStateForUnsupportedResolution() {
      ...
      if ("less_than_frequency" !== i) {
        let o = ["D", "W", "M"];
        return !0 !== (0, si.getChartingLibraryGlobalContext)().configurationData.is_tradingview_data && (o = e.supported_resolutions), {
          ticker: s,
          reason: i,
    ...
    _updateIsPrePostMarketPricesForbiddenProperty() {
      const e = this.symbolInfo(),
        t = (0, g.symbolHasPreOrPostMarket)(e) && (this.isDWM() || !!e && (0, g.isRegularSessionId)(this.sessionIdProxyProperty().value(), e));
    ...
    _getExtendedSymbolObject(e) {
      ...
      const s = this.unit();
      return this._model.unitConversionEnabled() && null !== s && (t["unit-id"] = s), !this.isDWM() && fi && (t.session = i.sessionId.value()), t
    }
    _checkChartStyle() {
      const e = this.style();
      (0, g.isCloseBasedSymbol)(this.symbolInfo()) || this.intervalObj().value().is1Tick() ? (0, g.isSingleValueBasedStyle)(e) || (this.requestingStyleIsNotSupported.fire(), this._styleToRecover = {
        correctedStyle: this.style(),
        originalStyle: e
      }) : "hlc" === this.symbolInfo()?.visible_plots_set ? (0, g.isHLCBasedStyle)(e) || (0, g.isSingleValueBasedStyle)(e) || (this.requestingStyleIsNotSupported.fire(), this._styleToRecover = {
        correctedStyle: this.style(),
        originalStyle: e
    ...
    _updateSymbolInfo(e) {
      if (this._symbolInfo.setValue(e), e) {
        ...
        "alwaysOff" === (0, J.currencyUnitVisibilityProperty)().value() || n === i.currencyId.value() && r === i.unitId.value() || this._model.fullUpdate(), i.currencyId.setValue(n), i.unitId.setValue(r);
        const a = this._getResolvingSymbolObject(),
          l = this._seriesSource.symbol();
        (0, s.default)(t, l) && !(0, s.default)(a, l) && this._seriesSource.updateExtSymbol(a), this._updateSessionIdProxyProperty(!0)
      }
      ...
      i !== this._quotesProvider.symbol() && this._quotesLastSplits.setValue(null), this._quotesProvider.setQuotesSessionSymbol(i), this._marketStatusModel.setSymbolInfo(e), e && this._recreatePriceFormattingDependencies(), this._statusView.update(), this.priceScale().updateFormatter(), this._symbolResolvingActive.setValue(!e), fi && this._updateIsPrePostMarketPricesForbiddenProperty()
    }
    ...
    _onSeriesCompleted(e) {
      ...
      this._seriesErrorMessage = null, this._updateCompleteStatusMode(e.updateMode), this._lastCompleteFlags = e.flags ?? null;
      ...
      if (null !== this._model.appliedTimeFrame().value() && i.lockVisibleTimeRangeOnResize(), this._model.invalidate(i), !this.priceScale().isLockScale() || this.model().timeScale().isEmpty() || this._seriesLoaded || (this.model().timeScale().correctOffset(), this.model().timeScale().correctBarSpacing(), this.model().resetPriceScale(t, this.priceScale())), this._seriesLoaded = !0, this._seriesCompleted = !0, this.model().recalcVisibleRangeStudies(h.RecalcVisibleRangeStudiesReason.SeriesCompleted),
        this.model().recalcStudyBasedLineTools(), null !== this._savedLeftEdge && 2 !== this._status.value().seriesStatus) {
    ...
    _saveLeftEdgeIfRequired(e, t = null) {
      if (this._savedLeftEdge = null, !t && !e && this._model.properties().childs().scalesProperties.childs().saveLeftEdge.value()) {
        const e = this._model.timeScale();
        if (!e.isEmpty()) {
          let t = e.visibleBarsStrictRange()?.firstBar();
          if (void 0 !== t) {
            const i = this._seriesSource.data()?.bars().first();
            return null != i && (t = Math.max(t, i.index)), this._savedLeftEdge = e.indexToTimePoint(t), this._pendingScrollToLeftEdge = (0, l.createDeferredPromise)(), !0
          }
        }
      }
      return !1
    ...
    async _applySymbolParamsChanges(e) {
      ...
      Q.Interval.isRange(t) && this._properties.childs().rangeStyle.childs().inputs.childs().range.setValue(Q.Interval.parse(t).multiplier());
      const {
        symbolChanged: i,
        intervalChanged: s,
        currencyChanged: o,
      ...
      if (this._syncModel = null, this._prevRequestedInterval = t, 5 !== this._status.value().seriesStatus && (!this._seriesSource.isStarted() || i || o || n || a) && this._updateSymbolInfo(null), this._processReplayOnApplySymbolParamsChanges(!!s, t), 5 === this._status.value().seriesStatus) return void this._model.realignLineTools();
      this._setStatus(1), fi && this._updateIsPrePostMarketPricesForbiddenProperty(), this._symbolIntervalChanged.fire(), s && this._notifyIntervalChanged(t), this._onRestarted.fire(), this._seriesLoaded = !1, this._seriesCompleted = !1, this._lineStyleLastPriceCirclePaneView.stopAnimation();
      let l = null;
      this._pendingTimeRange && (l = this._pendingTimeRange, this._pendingTimeRange = null), this._onTimeFrameApplied.fire(l), this._onBeforeModifySeries(this.getSymbolString(), t);
      const c = this._shouldDefineNumberOfBarsForModifySeries(e) && this._visibleBarsCount() || null;
      this.model().recalcVisibleRangeStudies(h.RecalcVisibleRangeStudiesReason.SeriesRestart), this._saveLeftEdgeIfRequired(!!i, l), this._data = null, this._seriesSource.modifySeries(this._getResolvingSymbolObject(), (0, Mt.getServerInterval)(t), l, r, c), this._seriesSource.isStarted() || (this._predictBars = 0, this._seriesSource.start()), (i || o || n) && this.disablePriceRangeReady(), this.updateAllViews((0, ve.sourceChangeEvent)(this.id())), this._model.lightUpdate(), this._pendingScrollToLeftEdge && await this._pendingScrollToLeftEdge.promise
    }
    ...
    _onSessionIdPropertyChanged() {
      this._sessionIdChanged.fire(), this.isDWM() || (this._saveLeftEdgeIfRequired(!1), this.restart(!0), this._updateLastPriceAnimationActive()), this._syncModel = null
    }
    ...
    _updateLastPriceAnimationActive() {
      ...
      this._lastPriceAnimationActive && e !== this._lastPriceAnimationActive && this.model().invalidate(_t.InvalidationMask.cursor())
    }
    _onPropertiesChanged(e) {
      const t = this._properties.childs();
      if (e === t.symbol || e === t.interval || e === t.timeframe) return;
      const i = this._id.value();
      this._tagsChanged.fire(), this.createPaneView(), this.updateAllViews((0, ve.sourceChangeEvent)(i)), this.model().updateSource(this), (0, a.emit)("series_properties_changed", i)
    }
    ...
    _onShowCountdownChanged(e) {
      ...
      }), 500) : null !== this._countdownUpdateTimer && (this._model.clearInterval(this._countdownUpdateTimer), this._countdownUpdateTimer = null)
    }
    ...
    _colorOnPrevBar() {
      const e = new Map([[1, "candleStyle"], [0, "barStyle"], [19, "volCandlesStyle"], [8, "haStyle"], [13, "columnStyle"]]).get(this.style());
