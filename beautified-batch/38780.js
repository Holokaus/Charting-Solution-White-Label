/**
 * Module 38780 - Auto-beautified from TradingView webpack bundle
 *
 * @module 38780
 * @date 2026-04-23
 * @size 9496 bytes
 ...
 *
 * Dependencies: 24640, 26709, 26867, 32563, 40167, 46021, 49483, 50151, 50470, 61814, 77914, 94194
 *
 * Exports:
 *   - hide (internal: K)
 *   - show (internal: G)
 *   - showOnElement (internal: j)
 *
 * Next Steps:
...
function I(e) {
  ...
  if (e.hotkey) {
    ...
    }(r.text, a).replace(/\s\+\s/g, `<span class="${x["common-tooltip__plus-sign"]}">+</span>`), i.classList.add(x["common-tooltip__body--with-hotkey"]), t && n.classList.add(x["common-tooltip__hotkey-block--divider"]), i.appendChild(n)
  }
...
function A(e, t) {
  ...
  ! function(e, t) {
    ...
    e.classList.remove(...w.map((e => b[e])).filter((e => !!e))), i && !e.classList.contains(i) && e.classList.add(i)
  }(e, t.colorTheme || "default"), t.addClass && e.classList.add(t.addClass);
  const s = O(e),
    o = e.querySelector(`.${x["common-tooltip__button-container"]}`);
  s.classList.toggle(x["common-tooltip__body--width_wide"], Boolean(t.wide)), s.classList.toggle(x["common-tooltip__body--no-padding"], Boolean(t.noPadding)), s.classList.toggle(x["common-tooltip__body--width_narrow"], Boolean(t.narrow)), s.classList.toggle(x["common-tooltip__body--no-buttons"], !0), s.style.left = k(0), s.style.width = k(s.clientWidth + (Boolean(t.noPadding) ? 0 : 2));
  const n = document.body.clientWidth,
    r = S.CheckMobile.iOS() || S.CheckMobile.Android() || (0, S.supportTouch)() && (0, S.isMac)() ? window.innerHeight : document.body.clientHeight,
    a = t.vertical,
    l = t.extendMargin || a && i.w < 20 || !a && i.h < 20;
  e.classList.toggle(x["common-tooltip--farther"], l), e.classList.toggle(x["common-tooltip--vertical"], a), e.classList.toggle(x["common-tooltip--horizontal"], !a);
  const c = function(e) {
  ...
  if (a) {
    const a = 10,
      ...
      m = p + h;
    e.style.left = k(i.x + i.w), e.style.top = k(p), p < a ? s.style.top = o.style.top = k(a - p) : m > l && (s.style.top = o.style.top = k(l - m));
    const {
      right: g
    } = (e.querySelector(":last-child") || s).getBoundingClientRect(), y = g + 10 > n;
    e.classList.toggle(x["common-tooltip--direction_reversed"], y), e.classList.toggle(x["common-tooltip--direction_normal"], !y);
    let S = y ? "after" : "before";
    (0, f.isRtl)() ? (S = t.otr ? "after" : S, S = t.otl ? "before" : S) : (S = t.otr ? "before" : S, S = t.otl ? "after" : S), c.classList.toggle(x["common-tooltip__ear-holder--before"], "before" === S), c.classList.toggle(x["common-tooltip__ear-holder--after"], "after" === S), "after" === S && (e.style.left = "auto", e.style.right = k(n - i.x))
  } else {
    ...
    e.style.left = k(u);
    const _ = d < a;
    e.classList.toggle(x["common-tooltip--direction_reversed"], _), e.classList.toggle(x["common-tooltip--direction_normal"], !_);
    ...
    "above" === p ? e.style.bottom = k(r - i.y) : e.style.top = k(i.y + i.h), c.classList.add("above" === p ? x["common-tooltip__ear-holder--above"] : x["common-tooltip__ear-holder--below"]);
    const {
      left: m
    } = s.getBoundingClientRect();
    let g = Math.trunc(i.x + i.w / 2 - (m + s.clientWidth / 2));
    e.style.left = k(u + g), e.style.width = k(s.clientWidth + o.clientWidth), g = _ ? Math.max(0, g) : Math.min(0, g), o.style.left = k(-g), s.style.left = k(-g)
  }
}
...
const U = new MutationObserver((() => {
    if (H && H.options.target) {
      let e;
      e = "isConnected" in H.options.target ? H.options.target.isConnected : document.body.contains(H.options.target), e || K()
    }
  })),
  j = (e, t = {}) => {
    const {
...
function q(e, t) {
  ...
  const i = function(e, t, i) {
    const s = [];
    for (; e && e !== t;) e.classList && e.classList.contains(i) && s.push(e), e = e.parentElement || Z(e.parentNode);
    return s
  ...
  for (const s of i) {
    ...
    if (i()) {
      const e = e => o(null, !0),
        o = (n, r = !1) => {
          s.removeEventListener("common-tooltip-update", i), s.removeEventListener("mouseleave", o), s.removeEventListener("mousedown", o), document.removeEventListener("scroll", e, {
            capture: !0
          }), t && (s.removeEventListener("blur", o), s.removeEventListener("active-descendant-blur", o)), z && (z.destroy(), z = null), K(r)
        };
      s.addEventListener("common-tooltip-update", i), s.addEventListener("mouseleave", o), s.addEventListener("mousedown", o), document.addEventListener("scroll", e, {
        capture: !0
      }), t && (s.addEventListener("blur", o), s.addEventListener("active-descendant-blur", o)), null === z && (z = (0, n.createGroup)({
        desc: "Tooltip"
      }), z.add({
        desc: "Hide",
        hotkey: 27,
