/**
 * Module 95059 - Auto-beautified from TradingView webpack bundle
 *
 * @module 95059
 * @date 2026-04-23
 * @size 8120 bytes
 ...
 *
 * Dependencies: 886, 1765, 3554, 9394, 11542, 13459, 17809, 27377, 34456, 37103, 38888, 43588, 45054, 46082, 48943, 49483, 52959, 55169, 55761, 59213, 63876, 67455, 67563, 69217, 76519, 82095, 83298, 83490, 88130, 88145, 91565, 92211, 92763, 93722, 98236, 99906
 *
 * Exports:
 *   - actualSymbol (internal: he)
 *   - chartStyleStudyId (internal: F)
 *   - createSeriesFormatter (internal: U)
 *   - displayedSymbolExchange (internal: ce)
 *   - displayedSymbolName (internal: le)
 *   - extractLineToolSymbolFromSymbolInfo (internal: se)
 *   - getChartStyleByResolution (internal: O)
 *   - getDefaultStyle (internal: N)
 *   - getLastUsedSingleValueBasedStyle (internal: R)
 *   - getLastUsedStyle (internal: V)
 *   - getSeriesDisplayErrorWV (internal: be)
 *   - getSeriesPriceFormattingState (internal: z)
 *   - getSourceForEconomySymbol (internal: re)
 *   - getSymbolDelaySeconds (internal: we)
 *   - getSymbolListedExchange (internal: ve)
 *   - getSymbolTradedExchange (internal: Se)
 *   - getTranslatedChartStyleName (internal: T)
 *   - hasVolume (internal: ge)
 *   - isCloseBasedSymbol (internal: _e)
 *   - isConvertedToOtherCurrency (internal: K)
 *   - isConvertedToOtherUnit (internal: J)
 *   - isEconomicSymbol (internal: fe)
 *   - isHLCBasedStyle (internal: D)
 *   - isMeasureUnitSymbol (internal: pe)
 *   - isPriceSourceStyle (internal: H)
 *   - isRangeBasedStyle (internal: x)
 *   - isRangeStyle (internal: I)
 *   - isRegularSessionId (internal: ue)
 *   - isSingleValueBasedStyle (internal: E)
 *   - isSpreadSymbolWithManyLegs (internal: ye)
 *   - isStyleSupportedForReplay (internal: L)
 *   - isTimeBasedStyle (internal: A)
 *   - isValidStyle (internal: k)
 *   - measureUnitId (internal: me)
 *   - preparePriceAxisProperties (internal: W)
 *   - proSymbol (internal: de)
 *   - setLastUsedStyle (internal: B)
 *   - styleChangeRequiresRestart (internal: M)
 *   - symbolCurrency (internal: G)
 *   - symbolCurrencyConvertible (internal: j)
 *   - symbolHasElectronicSession (internal: te)
 *   - symbolHasPreOrPostMarket (internal: ee)
 *   - symbolHasSeveralSessions (internal: ie)
 *   - symbolOriginalCurrency (internal: q)
 *   - symbolOriginalUnit (internal: X)
 *   - symbolTitle (internal: ae)
 *   - symbolTitleSeparator (internal: ne)
 *   - symbolToSaveInState (internal: oe)
 *   - symbolUnit (internal: Z)
 *   - symbolUnitConvertibleGroupsIfExist (internal: Q)
 *
 * Next Steps:
...
var v;
! function(e) {
  e.LastUsedStyleKey = "chart.lastUsedStyle", e.LastUsedSingleValueBasedStyle = "chart.lastUsedSingleValueBasedStyle", e.LastUsedRawDataStyle = "chart.rawDataStyle"
}(v || (v = {}));
const S = [4, 5, 6, 7, 8],
  b = [4, 5, 6, 7, 11],
...
function W(e) {
  const t = e.priceAxisProperties,
    i = t.lockScale.value(),
    s = 6 === e.style.value();
  (s || i) && (t.log.setValue(!1), t.percentage.setValue(!1)), t.logDisabled.setValue(!(!s && !i)), t.percentageDisabled.setValue(!(!s && !i))
}
...
function z(e, t = "default", i = !1) {
  ...
  if ("default" === t) null != e && (({
    ...
    let e = t.split(",");
    3 !== e.length && (e = ["100", "1", "false"]), a = parseInt(e[0]), l = parseInt(e[1]), s = "true" === e[2], r = !0
  }
  return i && (l = 1), {
    priceScale: a,
    minMove: l,
...
function ue(e, t) {
  return (0, p.getChartingLibraryGlobalContext)()?.configurationData?.is_tradingview_data ? ["stock", "fund", "index", "dr"].includes(t.type) && "regular" === e || (0, n.isFutures)(t.type) && "us_regular" === e : "regular" === e
}
...
function be(e) {
  return (0, u.combine)(((t, i) => {
    ...
    return null
  }), e.statusWV().weakReference(), (0, _.createWVFromGetterAndSubscription)((() => !e.bars().size() && !e.isInReplay().value()), e.dataEvents().dataUpdated()).ownership())
}
