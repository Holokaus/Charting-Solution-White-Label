/**
 * Module 23752 - Auto-beautified from TradingView webpack bundle
 *
 * @module 23752
 * @date 2026-04-23
 * @size 2231 bytes
 ...
 *
 * Dependencies: 33350, 52859
 *
 * Exports:
 *   - TimeAxisView (internal: r)
 *
 * Next Steps:
...
class n {
  ...
  draw(e, t, i) {
    ...
    if (s.alwaysInViewPort) {
      const e = s.width;
      h < 0 ? (c += Math.abs(0 - h), h = Math.floor(c - l) + .5) : h + a > e && (c -= Math.abs(e - (h + a)), h = Math.floor(c - l) + .5)
    }
    const d = h + a,
    ...
    const m = Math.round(h * _),
      ...
      y = Math.round(u * p),
      v = Math.round(2 * _);
    e.beginPath(), e.moveTo(m, g), e.lineTo(m, y - v), e.arcTo(m, y, m + v, y, v), e.lineTo(f - v, y), e.arcTo(f, y, f, y - v, v), e.lineTo(f, g), e.fill();
    ...
    e.translate((h + r) * _, (S + b) * p), (0, o.drawScaled)(e, _, p, (() => e.fillText(s.text, 0, 0))), e.restore()
  }
}
class r {
  constructor(e) {
  ...
  _updateImpl() {
    ...
    null !== t && Number.isFinite(t) && (e.visible = !0, e.width = this._model.timeScale().width(), e.background = this._getBgColor(), e.color = (0, s.colorFromBackground)(e.background), e.coordinate = this._model.timeScale().indexToCoordinate(t), e.alwaysInViewPort = this._getAlwaysInViewPort(), e.text = this._getText(t), this._invalidated = !1)
  }
