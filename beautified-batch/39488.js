/**
 * Module 39488 - Auto-beautified from TradingView webpack bundle
 *
 * @module 39488
 * @date 2026-04-23
 * @size 5782 bytes
 ...
 *
 * Dependencies: 30798, 58554, 60661, 69866, 82130, 82284, 87465, 99481
 *
 * Exports:
 *   - StaticStudyGraphics (internal: d)
 *   - emptyStudyGraphics (internal: g)
 *   - loadStudyGraphics (internal: f)
 *   - saveStudyGraphics (internal: y)
 *
 * Next Steps:
...
function y(e, t) {
  const i = function(e) {
      ...
      e.horizlines().forEach(((e, i) => {
        ...
        e.forEach((e => {
          e.forEach((e => {
            t.add(e.left ?? s.INVALID_TIME_POINT_INDEX), t.add(e.right ?? s.INVALID_TIME_POINT_INDEX)
          }))
        }))
      })), e.tpos().forEach((e => {
        e.forEach((e => {
        ...
        e.forEach((e => {
          e.forEach((e => {
            i(e.poorHighExtendTo), i(e.poorLowExtendTo), i(e.tpoPocExtendTo), (e.singleprints ?? []).map((e => e.extendTo)).map(i)
          }))
        }))
