...
19844: (watchedValue_e, watchedValue_t, i) => {
    "use strict";
    i.d(watchedValue_t, {
      StudyMetaInfo: () => S,
      getStudyIdWithVersion: () => v
    });
    var watchedValue_s = i(83873),
      o = i(90054),
      watchedValue_n = i(87465),
      r = i(9343),
      watchedValue_a = i(18113),
      l = i(4359),
      c = i(69422),
      h = i(60973),
      d = i(44862);
    ...
    class S {
      constructor(watchedValue_e, watchedValue_t = !1) {
        ...
        const i = watchedValue_e.fullId || watchedValue_e.id;
        (0, watchedValue_n.merge)(this, S.parseIdString(i)), this._updateInputDisplayDefaults(), this.useVersionFromMetaInfo = watchedValue_t
      }
      defaultInputs() {
      ...
      state() {
        ...
        for (const [watchedValue_t, i] of Object.entries(this)) "useVersionFromMetaInfo" !== watchedValue_t && this.hasOwnProperty(watchedValue_t) && (watchedValue_e[watchedValue_t] =
          (0, o.default)(i), "id" === watchedValue_t && (watchedValue_e[watchedValue_t] += "-" + this.version));
      ...
      createDefaults() {
        if (this.defaults) {
          ...
          (0, h.createDefaults)(watchedValue_t, watchedValue_e)
        }
      }
      removeDefaults() {
      ...
      isPlotForceOverlay(watchedValue_e) {
        const watchedValue_t = this.styles?.[watchedValue_e],
          i = this.plots.find((watchedValue_t => watchedValue_t.id === watchedValue_e));
        return !!watchedValue_t?.forceOverlay || i && (0, l.isOhlcPlot)(i) && this.ohlcPlots?.[i.target]?.forceOverlay || !!this
      ...
      static getSourceIdsByInputs(watchedValue_e, watchedValue_t) {
        ...
        const i = [];
        for (const o of watchedValue_e)
          if (S.isSourceInput(o) && (0, watchedValue_s.default)(watchedValue_t[o.id])) {
            const watchedValue_e = watchedValue_t[o.id];
            watchedValue_e.includes("$") && i.push(watchedValue_e.split("$")[0])
          } return i
      }
      ...
      static getSourceInputIds(watchedValue_e) {
        ...
        for (const i of watchedValue_e.inputs) S.isSourceInput(i) && watchedValue_t.push(i.id);
        return watchedValue_t
      }
      static setChildStudyMetaInfoPropertiesSourceId(watchedValue_e, watchedValue_t, i) {
        for (const watchedValue_s of watchedValue_e.inputs) {
          if (!S.isSourceInput(watchedValue_s)) continue;
          const watchedValue_e = i.childs().inputs && i.childs().inputs.childs()[watchedValue_s.id];
          if (watchedValue_e) {
            const i = watchedValue_e.value(),
              watchedValue_s = f.exec(i);
            if (2 === watchedValue_s?.length) {
              if ("{pid}" === watchedValue_s[1]) {
                const watchedValue_s = i.replace(/^[^\$]+/, watchedValue_t);
      ...
      static canBeChild(watchedValue_e) {
        ...
        for (const i of watchedValue_e.inputs) S.isSourceInput(i) && (watchedValue_t += 1);
      ...
      static canHaveChildren(watchedValue_e) {
        if (watchedValue_e) {
          ...
          if (watchedValue_e.id && !p.has(watchedValue_e.id) && Array.isArray(watchedValue_e.plots))
            for (const watchedValue_t of watchedValue_e.plots)
              if (m.has(watchedValue_t.type)) return !0
        }
        return !1
      }
      static getChildSourceInputTitles(watchedValue_e, watchedValue_t, i) {
        ...
        if (watchedValue_t.plots && watchedValue_t.plots.length && watchedValue_e.options && watchedValue_e.options.length)
          for (const o of watchedValue_e.options) {
            const watchedValue_e = o ? +o.split("$")[1] : NaN,
              watchedValue_n = isFinite(watchedValue_e) && watchedValue_t.plots[watchedValue_e];
            watchedValue_n && m.has(watchedValue_n.type) && (watchedValue_s[o] = watchedValue_t.styles && watchedValue_t.styles[watchedValue_n.id] && watchedValue_t.styles[watchedValue_n.id]?.title || watchedValue_n.id, i && (watchedValue_s[o] =
              i + ": " + watchedValue_s[o]))
          }
        return watchedValue_s
      }
      static canPlotBeSourceOfChildStudy(watchedValue_e) {
        return m.has(watchedValue_e)
      }
      static getStudyPropertyRootName(watchedValue_e) {
        const watchedValue_t = g.get(watchedValue_e.id);
        if (void 0 !== watchedValue_t) return watchedValue_t;
        let i = "study_" + watchedValue_e.id;
        return watchedValue_e.pine && watchedValue_e.pine.version && (i += "_" + watchedValue_e.pine.version.replace(".", "_")), i
      }
      static getStudyPropertyRootNameById(watchedValue_e) {
        const watchedValue_t = g.get(watchedValue_e);
      ...
      static parseIdString(watchedValue_e) {
        return function(watchedValue_e) {
          ...
          if (-1 === watchedValue_e.indexOf("@")) watchedValue_t.shortId = watchedValue_e, watchedValue_t.packageId = "tv-basicstudies", watchedValue_t.id = watchedValue_e + "@" + watchedValue_t.packageId, watchedValue_t
            ...
            const i = watchedValue_e.split("@");
            watchedValue_t.shortId = i[0];
            const watchedValue_s = i[1].split("-");
          ...
          if (watchedValue_t.fullId = watchedValue_t.id + "-" + watchedValue_t.version, "tv-scripting" === watchedValue_t.packageId) {
            ...
            if (0 === watchedValue_e.indexOf("Script$") || 0 === watchedValue_e.indexOf("StrategyScript$")) {
              const i = watchedValue_e.indexOf("_");
              watchedValue_t.productId = i >= 0 ? watchedValue_e.substring(0, i) : watchedValue_t.packageId
      ...
      static versionOf(watchedValue_e) {
        ...
        return watchedValue_t < 0 && u.logError("Metainfo format version cannot be negative: " + watchedValue_t), watchedValue_t
      }
      static getPackageName(watchedValue_e) {
        ...
        watchedValue_t?.[1] || (0, watchedValue_a.getPersistentLogger)()?.addPersistentLogEntry(
          `getStudyPackageName: study id ${watchedValue_e} with stack ${(new Error).stack}`, r.LOGLEVEL.INFO,
          "chart.StudyMetaInfo");
        const [, i = "tv-basicstudies"] = watchedValue_t || [];
        return i
      }
      static cutDollarHash(watchedValue_e) {
        const watchedValue_t = watchedValue_e.indexOf("$");
        if (-1 === watchedValue_t) return watchedValue_e;
        const i = watchedValue_e.indexOf("@"),
          watchedValue_s = i >= 0 ? watchedValue_e.substring(i) : "";
      ...
      static getStudyIdWithLatestVersion(watchedValue_e, watchedValue_t) {
        const i = S.cutDollarHash(watchedValue_e.id);
        let watchedValue_s = i;
        return i.indexOf("Portfolio@tv-scripting") >= 0 ? watchedValue_s += watchedValue_t ? "!" : "" : "ReplayStrategy@tv-scripting" === watchedValue_s ?
          watchedValue_s += "-581!" : i.indexOf("@tv-scripting") >= 0 ? watchedValue_s += "-101!" : i.endsWith("CP@tv-basicstudies") ? watchedValue_s +=
          "-" + Math.min(watchedValue_e.version, 207) : i.endsWith("CP@tv-chartpatterns") ? watchedValue_s += "-" + Math.min(watchedValue_e.version, 9) : i
      ...
      static overrideDefaults(watchedValue_e) {
        0 !== watchedValue_e.length && (0, d.applyOverridesToStudyDefaults)(this.savedStudiesOverrides, watchedValue_e, (watchedValue_e => window.TradingView
      ...
      static findStudyMetaInfoByDescription(watchedValue_e, watchedValue_t) {
        if (watchedValue_e) {
          const i = watchedValue_t.toLowerCase();
          for (const watchedValue_t of watchedValue_e)
            if (watchedValue_t.description.toLowerCase() === i) return watchedValue_t;
      ...
      _updateInputDisplayDefaults() {
        this.inputs.filter((watchedValue_e => void 0 === watchedValue_e.display)).forEach((watchedValue_e => {
          y.includes(watchedValue_e.type) ? watchedValue_e.display = c.InputDisplayFlags.None : watchedValue_e.display = c.InputDisplayFlags.All
        }))
      }
    }
